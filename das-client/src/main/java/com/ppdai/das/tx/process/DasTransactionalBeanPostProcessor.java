...
@Component
public class DasTransactionalBeanPostProcessor implements BeanPostProcessor, Ordered , InitializingBean {
    ...
    public Method getConfirm(String name) {
        NodeMeta meta = metas.get(name);
        Preconditions.checkNotNull(meta, "Cannot find method " + name);
        return meta.getConfirmMethod();
    }

    public void callConfirm(String name) throws InvocationTargetException, IllegalAccessException {
        NodeMeta meta = metas.get(name);
        Preconditions.checkNotNull(meta, "Cannot find method " + name);
        meta.getConfirmMethod().invoke(meta.getTarget());
    }

    public Method getCancel(String name) {
        NodeMeta meta = metas.get(name);
        Preconditions.checkNotNull(meta, "Cannot find method " + name);
        return meta.getCancelMethod();
    }

    public void callCancel(String name) throws InvocationTargetException, IllegalAccessException {
        NodeMeta meta = metas.get(name);
        Preconditions.checkNotNull(meta, "Cannot find method " + name);
        meta.getCancelMethod().invoke(meta.getTarget());
    }
